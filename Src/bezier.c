...
File name:      bezier.c
Description:    外部调用track_bezier_init（）即可把贝塞尔曲线上的点传到底盘将要
                走的坐标上，修改输入的控制点在track.c中
Author:         徐铭远
Version：       1.0
...
void track_bezier_init()
{
    ...
    chassis_posnum=out_num;
//以下打印为了给matlab生成m文件用，可以忽略
/*    uprintf(CMD_USART,"close all;\r\n");
    uprintf(CMD_USART,"x=[\r\n");
    for(int j=0; j<input_num; j++)    // 输出控制点x 
    {
		uprintf(CMD_USART,"%f ",in[j].X);
    }
    uprintf(CMD_USART,"];\r\n");
    uprintf(CMD_USART,"y=[\r\n");
    for(int j=0; j<input_num; j++)    // 输出控制点y 
    {
		uprintf(CMD_USART,"%f ",in[j].Y);
    }
    uprintf(CMD_USART,"];\r\n");
    
    uprintf(CMD_USART,"plot(x,y,':');\r\n");
    uprintf(CMD_USART,"hold on;\r\n");
    
    uprintf(CMD_USART,"x=[\r\n");
    for(int j=0; j<(input_num*2-4); j++)    // 输出控制点x 
    {
		uprintf(CMD_USART,"%f ",ctrl_points[j].X);		
    }
    uprintf(CMD_USART,"];\r\n");
    uprintf(CMD_USART,"y=[\r\n");
    for(int j=0; j<(input_num*2-4); j++)    // 输出控制点y 
    {
		uprintf(CMD_USART,"%f ",ctrl_points[j].Y);
    }
    uprintf(CMD_USART,"];\r\n");
    ...
    {
    	if(j!=0)
        	uprintf(CMD_USART,"%f ",out[j].Y);
        if(j%10==0&&j!=0)
        	uprintf(CMD_USART,"\r\n");
    }
    uprintf(CMD_USART,"];\r\n");
	  
	uprintf(CMD_USART,"plot(x,y,'r');");
*/
}